...
void M_TooOld()
{
	if (RegistryLanguage("Russian")) MessageBox(0, "К сожалению, ваша версия игры слишком старая для того, чтобы использовать D3D9Wnd. Пожалуйста, обновите игру до последней версии.", "У нас проблема, капитан!", MB_OK | MB_ICONERROR);
...
{
	if (GameLanguage("Russian")) MessageBox(0, "Аппаратный курсор мыши был отключен, чтобы некоторые опции работали правильно.", "Оповещение модуля D3D9Wnd", MB_OK | MB_ICONINFORMATION);
...
{
	if (RegistryLanguage("Russian")) MessageBox(0, "Для правильной работы растянутого/центрированного/пользовательского меню следует отключить опцию \"Аппаратный курсор мыши\" в дополнительных настройках игры.", "Оповещение модуля D3D9Wnd", MB_OK | MB_ICONWARNING);
...
{
	if (RegistryLanguage("Russian")) Mprintfi("Оповещение модуля D3D9Wnd", "Привет, и спасибо, что пользуетесь опцией мультимониторного отображения игры. Я задал разрешение вашей игры на %ux%u. Пожалуйста, перезапустите игру, чтобы подтвердить изменения.", nWidth, nHeight);
...
{
	if (RegistryLanguage("Russian")) Mprintfe("Ошибка модуля D3D9Wnd", "Извините, но глубина цвета в вашей системе (%u битов на пиксель) слишком низкая для того, чтобы использовать D3D9Wnd. Пожалуйста, задайте глубину цвета хотя бы на 32 бита на пиксель.", dwDepth);
